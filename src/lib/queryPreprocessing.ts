...
export function extractQueryWords(query: string): string[] {
  ...
  const meaningfulWords = words.filter(word => !NOISE_WORDS.includes(word));

  return meaningfulWords.length > 0 ? meaningfulWords : words;
}

export function fuzzyMatchText(query: string, text: string, threshold: number = 0.7): number {
  ...
  return averageSimilarity * 0.7 + coverageBonus * 0.3;
}

export function generateFuzzySearchTerms(query: string): string[] {
  const terms: string[] = [];
