...
export default function Comment({ comment, postId, onUpdate, isReply = false }: CommentProps) {
  ...
  const fetchCommentData = async () => {
    const [authorRes, reactionRes, userReactionRes] = await Promise.all([
      ...
      supabase.from('reactions').select('*').eq('target_id', comment.id).eq('target_type', 'comment'),
      hubookProfile
        ? supabase
            .from('reactions')
            .select('*')
