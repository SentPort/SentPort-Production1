/*
  # Fix bulk_add_content_type_rules Not Returning Results

  ## Problem
  The function successfully adds domains to content_type_domain_rules table,
  but doesn't return any rows to the caller. The RETURN QUERY SELECT is not executing.

  ## Root Cause
  The EXCEPTION handler uses CONTINUE which skips the RETURN QUERY.
  Even when INSERT succeeds, if ANY error occurs afterward (like in the COUNT query),
  the CONTINUE statement skips the RETURN QUERY entirely.

  ## Solution
  Move the RETURN QUERY outside the exception-prone block and ensure it always executes
  when the INSERT/UPDATE succeeds. Use a success flag to track if we should return.
*/
