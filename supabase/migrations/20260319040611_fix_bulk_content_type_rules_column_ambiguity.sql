/*
  # Fix Column Ambiguity in Bulk Content Type Rules

  ## Problem
  Variable name "domain" conflicts with table column name "domain"
  causing ambiguous reference error.

  ## Solution
  Qualify all column references with table name to avoid ambiguity.

  ## Changes
  - Add table qualifiers to all domain column references
*/
...
BEGIN
  ...
  FOREACH v_domain IN ARRAY p_domains LOOP
    ...
    -- Check if exists (qualify column name)
    v_existed := EXISTS(
