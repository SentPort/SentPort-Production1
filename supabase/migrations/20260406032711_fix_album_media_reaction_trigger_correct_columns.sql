/*
  # Fix Album Media Reaction Trigger - Correct Column Names

  1. Problem
    - The notify_media_owner_on_reaction function tries to insert into non-existent columns: 'title' and 'link'
    - The hubook_notifications table schema has: user_id, type, actor_id, post_id, comment_id, 
      share_id, album_media_comment_id, message, read, dismissed, created_at
    
  2. Solution
    - Update INSERT statement to use correct columns that actually exist
    - Use 'actor_id' for the reactor's user ID
    - Use 'message' for the notification text
    - Set unused foreign key columns to NULL
    - Remove references to 'title' and 'link' which don't exist

  3. Security
    - Maintains SECURITY DEFINER for proper RLS bypass during notification creation
*/

-- Fix the notification trigger function to use correct schema
