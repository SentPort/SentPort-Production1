/*
  # Fix Ambiguous Column Reference in bulk_add_content_type_rules

  ## Problem
  The EXISTS query has an ambiguous column reference because "domain" 
  could refer to either the table column or the RETURNS TABLE output column.

  ## Solution
  Use explicit table qualification: content_type_domain_rules.domain
*/
...
BEGIN
  ...
  LOOP
    ...
    -- Check if domain already exists BEFORE insert (use table qualification)
    SELECT EXISTS(
