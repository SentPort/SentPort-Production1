...
CREATE TABLE IF NOT EXISTS album_media_tags (
  ...
  tagged_user_id uuid NOT NULL REFERENCES user_profiles(user_id) ON DELETE CASCADE,
  tagger_id uuid NOT NULL REFERENCES user_profiles(user_id) ON DELETE CASCADE,
...
CREATE POLICY "Tagger and media owner can delete tags"
  ...
  USING (
    ...
    OR EXISTS (
      SELECT 1 FROM album_media am
      WHERE am.id = media_id AND am.user_id = auth.uid()
    )
  );
